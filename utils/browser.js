...
try {
	...
	console.warn(
		'[Browser] Camoufox no pudo cargarse, se usará Playwright Firefox estándar como respaldo:',
		e.message,
	)
}
...
export async function launchBrowser(options = {}) {
	// Always use local Puppeteer (or puppeteer-core in production).
	const isProduction =
	...
	if (isProduction) {
		try {
			...
			console.error(
				'[Browser] Error al lanzar puppeteer-core en producción:',
				error.message,
			)
		}
	}
...
export async function launchCamoufox(options = {}) {
	...
	if (Camoufox) {
		...
		try {
			...
			console.error(
				'[Browser] Error al lanzar Camoufox, intentando Playwright Firefox:',
				error.message,
			)
		}
	}
